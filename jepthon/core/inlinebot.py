...
def main_menu():
    text = f"**- [سـورس انتيثون](https://t.me/lDDDDV)\
        \n المسـاعد\
        \n\
        \n◽ انتيثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("jepiq"):
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس انتيثون 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./lDDDDV", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس انتيثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./lDDDDV", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس انتيثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/lDDDDV"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/lDDDDG",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            ...
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/lDDDDG)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
