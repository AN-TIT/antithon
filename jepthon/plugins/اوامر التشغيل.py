...
async def _(event):
    "Restarts the bot !!"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**⌔︙انتيثون العرب ↻** \n" "**⌯︙تم تحديث السورس بنجاح ✅ ↻**")
    lMl10l = await edit_or_reply(
        event,
        "**⌯︙جـاري تحديث سورس انتيثون ، قـد يستغـرق الأمـر 2-3 دقائـق لاتقم بكتابة تحديث مـره اخـرى انتـظـر ⏱**",
    )
    try:
